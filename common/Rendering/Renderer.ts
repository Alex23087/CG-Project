...
export class Renderer {
	...
	public constructor(canvas: HTMLCanvasElement) {
		...
		this.postProcessingShader.compile()
	...
	private drawFullscreenQuad(shader: PostProcessingShader | GaussianBlurShader, destinationFramebuffer: Framebuffer, previousFramebuffer: Framebuffer) {
		...
		if (shader instanceof PostProcessingShader) {
			...
			glMatrix.mat4.mul(vp, this.projectionMatrix, this.viewMatrix)
			let invVP = glMatrix.mat4.create();
	...
	startRendering = (time: number) => {
		...
		if (this.currentCamera) {
			...
			// Setup original camera back so that we can use view and projection matrices in post processing shaders
			this.viewMatrix = this.currentCamera.viewMatrix;
			this.projectionMatrix = this.currentCamera.projectionMatrix(this.fov, this.viewportSize.x / this.viewportSize.y)
	...
	private makeDefaultFramebuffer() {
		...
		this.gl.deleteFramebuffer(defaultFB.depthbuffer)
